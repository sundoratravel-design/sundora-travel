...
import re, os
PUBLIC_DIR = "public"
# KAYNAK dosya hiçbir zaman değiştirilmez — sadece okunur
SOURCE_PATH = os.path.join(PUBLIC_DIR, "index_kaynak.html")
# ÇIKTI dosyası — bu yazılır
INDEX_PATH  = os.path.join(PUBLIC_DIR, "index.html")

def read_source():
    with open(SOURCE_PATH, "r", encoding="utf-8") as f: return f.read()
...
def update_exp_section_text(html):
    ...
    html = re.sub(
        r'(href=["\'])([^"\']*?)(["\'][^>]*>Tüm Rotalar →)',
        r'\g<1>/butik-grup-turlari.html\g<3>', html
    )
    return html
...
#nav{background:transparent !important;backdrop-filter:none !important;border-bottom:none !important;position:fixed !important;top:0;left:0;right:0;z-index:300;transition:background .45s ease,backdrop-filter .45s ease,border-color .45s ease}
...
#nav .nav-cta{border-color:rgba(222,200,152,.6) !important;color:rgba(222,200,152,.95) !important}
#nav .nav-logo img{filter:brightness(0) invert(1) !important;opacity:.92 !important}
#nav.s{background:rgba(18,12,6,.92) !important;backdrop-filter:blur(24px) !important;border-bottom:1px solid rgba(196,160,104,.22) !important}
...
body:not(.page-home) #nav{background:rgba(18,12,6,.88) !important;backdrop-filter:blur(20px) !important;border-bottom:1px solid rgba(196,160,104,.2) !important}
.sd-video-hero{position:relative;width:100%;height:100vh;min-height:600px;overflow:hidden;background:#0a0806}
.sd-vid{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity 1.6s cubic-bezier(.4,0,.2,1);z-index:1}
.sd-vid.sd-vid-active{opacity:1;z-index:2}
.sd-vid-overlay{position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,.28) 0%,rgba(0,0,0,.08) 40%,rgba(0,0,0,.45) 100%);z-index:3}
.sd-vid-dots{position:absolute;bottom:2.2rem;left:50%;transform:translateX(-50%);display:flex;gap:.6rem;z-index:6}
.sd-dot{width:28px;height:2px;background:rgba(255,255,255,.35);border-radius:2px;cursor:pointer !important;transition:background .4s,width .4s}
.sd-dot.sd-dot-active{background:rgba(255,255,255,.9);width:44px}
.sd-reveal{opacity:0;transform:translateY(32px);transition:opacity .75s cubic-bezier(.4,0,.2,1),transform .75s cubic-bezier(.4,0,.2,1)}
...
.mob-nav-overlay{position:fixed;inset:0;background:rgba(12,8,4,.97);backdrop-filter:blur(28px);z-index:299;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2.8rem;opacity:0;pointer-events:none;transition:opacity .4s cubic-bezier(.4,0,.2,1)}
.mob-nav-overlay.open{opacity:1;pointer-events:all}
.mob-nav-overlay a{font-family:'Cormorant Garamond',Georgia,serif;font-size:clamp(30px,6vw,46px);font-weight:300;color:rgba(255,248,235,.88);text-decoration:none;letter-spacing:.04em;transition:color .3s}
.mob-nav-overlay a:hover{color:#C4A068}
.mob-nav-overlay .mob-cta{font-family:'Jost',sans-serif;font-size:10px;font-weight:300;letter-spacing:.24em;text-transform:uppercase;padding:.9rem 2.8rem;border:1px solid rgba(196,160,104,.55);color:#DEC898;margin-top:1.2rem}
...
body.page-butik .page-title-block .sh,body.page-butik .page-title-block .lbl,body.page-butik .ect,body.page-butik .ecs,body.page-butik .ecn{color:#fff !important}
...
body.page-rotalar .page-title-block .sh,body.page-rotalar .page-title-block .lbl{color:#1a3a15 !important}
...
body.page-rotalar .filter-group select,body.page-rotalar .filter-group input[type="date"]{border-bottom-color:rgba(26,58,21,.4) !important;color:#1a3a15 !important}
...
@media(max-width:768px){.tour-grid{grid-template-columns:1fr 1fr}.contact-grid{grid-template-columns:1fr;gap:3rem}.about-grid{grid-template-columns:1fr;gap:3rem}.filter-bar{flex-direction:column}.page-section{padding:2rem 1.5rem 5rem}.page-title-block{padding:8rem 0 2rem}.form-row{grid-template-columns:1fr}.sd-video-hero{height:100svh}}
...
(function(){
  ...
  dots.forEach(function(d,i){d.addEventListener('click',function(){clearInterval(timer);goTo(i);timer=setInterval(function(){goTo(current+1);},INTERVAL);});});
  document.querySelectorAll('a[href]').forEach(function(a){
    var href=a.getAttribute('href');
    if(!href||href.startsWith('#')||href.startsWith('mailto')||href.startsWith('http'))return;
    a.addEventListener('click',function(e){e.preventDefault();document.body.style.transition='opacity .45s';document.body.style.opacity='0';setTimeout(function(){window.location.href=href;},420);});
  });
})();
(function(){
  var els=document.querySelectorAll('.sd-reveal');
  if(!els.length)return;
  var io=new IntersectionObserver(function(entries){entries.forEach(function(e){if(e.isIntersecting){e.target.classList.add('sd-visible');io.unobserve(e.target);}});},{threshold:0.12,rootMargin:'0px 0px -40px 0px'});
...
def main():
    print("\n🌍  Sundora Travel — Sayfa Üretici v7 (Kaynak Korumalı) Başlatıldı\n")
    if not os.path.exists(PUBLIC_DIR):
        print("HATA: public/ klasörü bulunamadı."); return
    if not os.path.exists(SOURCE_PATH):
        print(f"HATA: {SOURCE_PATH} bulunamadı. Önce yedekten kopyalayın."); return
    print(f"📖  {SOURCE_PATH} okunuyor... (kaynak dosya korunuyor)")
    html = read_source()
    ...
    print("\n✅  Tüm sayfalar güncellendi! (v7 - Kaynak Korumalı)\n")
